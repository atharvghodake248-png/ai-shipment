      ...
      <Card className="flex-1 overflow-hidden flex flex-col">
        <CardContent className="flex-1 overflow-y-auto p-4 space-y-4">
          {messages.filter(m => m.role === 'assistant').length === 0 && !isLoading && (
            <div className="text-center text-muted-foreground text-sm mt-10">Starting AI clarification...</div>
          )}
          {messages.map(m => m.role === 'assistant' && (
            <div key={m.id} className="flex gap-3">
              ...
              <div className="max-w-[80%] rounded-2xl px-4 py-3 text-sm bg-slate-100">{m.content}</div>
            </div>
          ))}
          {messages.map(m => m.role === 'user' && messages.indexOf(m) > 0 && (
            <div key={m.id + '_u'} className="flex gap-3 justify-end">
              <div className="max-w-[80%] rounded-2xl px-4 py-3 text-sm bg-primary text-primary-foreground">{m.content}</div>
            </div>
          ))}
          {isLoading && (
