        ...
        <div className="flex items-start justify-between mb-10">
          ...
          <Button onClick={() => generateTasks.mutate({ featureId })} disabled={generateTasks.isPending}
            ...
            {generateTasks.isPending
              ...
              : <><Sparkles className="w-4 h-4 mr-2" />{tasks?.length ? 'Regenerate' : 'Generate Tasks'}</>}
          </Button>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {COLUMNS.map((col) => {
              ...
              return (
                <div key={col.id}
                  className={`rounded-2xl border-2 ${col.border} ${col.bg} p-4 min-h-[500px] transition-all duration-200`}
